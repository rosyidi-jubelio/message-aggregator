...
export const processMessages = async () => {
  ...
  const orderMsgCollection = db.collection("order_message");

  const messages: OrderMessage[] = await orderMsgCollection.aggregate([
    {
      $group: {
      ...
      await Promise.map(
        ...
        async (chunk) => {
          try {
            ...
            logger.info(
              `Published message to RMQ, store_id: ${message.store_id}, channel_id: ${message.channel_id}, ref_nos: ${chunk.length}`
            );
          } catch (error) {
            logger.error(
